...
let intentHandlers = {
  ...
  getEventsRecommendations: function() {
    this.emit("sGetEventsRecommendations");
  },
    ...
    if (this.attributes[versionIdKey]) {
      ...
      s3Helper.getObject(this.attributes[fileNameKey], null, (err, data) => {
        if (err) {
          ...
          let timeSinceLastModified =
            new Date().getTime() - new Date(data.LastModified);
  ...
  readItems: function() {
    ...
    loadItems.call(this, () => {
      ...
      if (!this.attributes[feedEndedKey]) {
        ...
        let currentPaginationEnd =
          currentIndex + config.number_feeds_per_prompt;
        for (
          index = currentIndex;
          index < currentPaginationEnd && index < feedLength;
          index++
        ) {
  ...
  readPreviousItems: function() {
    ...
    loadItems.call(this, () => {
      ...
      if (!this.attributes[justStartedKey]) {
        ...
        for (
          index = currentIndex;
          index >= currentPaginationStart && index >= 0;
          index--
        ) {
        ...
        favoriteCategories.forEach(category => {
          ...
          asyncDataCollection.push(asyncCallback => {
            if (this.attributes[versionIdKey]) {
              ...
              s3Helper.getObject(
                this.attributes[fileNameKey],
                this.attributes[versionIdKey],
                (err, data) => {
                  if (err) {
              ...
              s3Helper.getObject(
                ...
                (err, data) => {
                  if (err) {
                    ...
                    let allowedTimePeriod =
                      constants.updateFeedTime * 60 * 1000;
...
function newItemNotification(callback) {
  ...
  if (
    ...
    this.attributes["latestItem"][category]
  ) {
...
function deleteFavoriteFile(callback) {
  ...
  if (
    this.attributes["latestItem"] &&
    this.attributes["latestItem"]["Favorite"]
  ) {
  ...
  if (this.attributes["favoriteFilePresent"]) {
    ...
    s3Helper.deleteObject(
      this.attributes["fileNameFavorite"],
      this.attributes["versionIdFavorite"],
      err => {
        if (err) {
          logHelper.logAPIError(this.event.session, "S3", err);
          this.emit("reportError");
