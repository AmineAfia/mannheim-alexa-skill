...
const speechHandlers = {
  ...
  illegalCategory: function() {
    ...
    let message =
      "Sorry, I could not understand. You can select any of the following categories : ";
  ...
  listFavorite: function() {
    ...
    if (favoriteListLength === 0) {
      ...
      for (let index = 0; index < favoriteListLength; index++) {
        message +=
          index + 1 + " . " + favoriteList[index] + constants.breakTime["300"];
  ...
  favoriteAddExistingError: function(category) {
    ...
    let message =
      category +
      " is already present in your favorites. " +
      "You can say, open favorites, to hear your favorite feeds.";
    const reprompt =
      "You can say, open favorites, to hear your favorite feeds.";
  ...
  favoriteRemoveExistingError: function(category) {
    ...
    let message =
      category +
      " was not present in your favorites. " +
      "You can say, open favorites, to hear your favorite feeds.";
    const reprompt =
      "You can say, open favorites, to hear your favorite feeds.";
    this.response.speak(message).listen(reprompt);
  ...
  favoriteListEmpty: function() {
    const message =
      "You have no feed marked as favorite. " +
      "You can say mark " +
      Object.keys(config.feeds)[0] +
      "  as favorite.";
    const reprompt =
      "Your list of favorite feeds is empty. " +
      "You can say mark " +
      Object.keys(config.feeds)[0] +
      "  as favorite.";
    this.response.speak(message).listen(reprompt);
  ...
  readPagedItems: function(items) {
    ...
    if (
      this.attributes["newItemCount"] &&
      this.attributes["newItemCount"] > 0
    ) {
      let msg;
    ...
    items.forEach(function(feed) {
      content +=
        config.speech_style_for_numbering_feeds +
        " " +
        (feed.count + 1) +
        ". " +
        feed.title +
  ...
  readPagedItemsSingleMode: function(items) {
    ...
    if (this.attributes["newItemCount"]) {
      ...
      if (this.attributes["newItemCount"] > 0) {
        ...
        cardContent =
          "There are " + this.attributes["newItemCount"] + msg + "\n";
        message +=
          this.attributes["category"] +
          " has " +
          this.attributes["newItemCount"] +
  ...
  helpSingleFeedMode: function() {
    const message =
      config.welcome_message +
      "To navigate through the feed, you can say commands like next and previous.";
    this.response.speak(message).listen(message);
    this.emit(":responseReady");
  },
  ...
  itemInfoError: function() {
    ...
    const message =
      "Sorry, you need to select a feed before requesting item details. " +
      "What would you like to hear?";
    let reprompt =
      "You can select any feed using its name or number" +
      constants.breakTime["100"] +
      "Here are the available feeds : " +
      constants.breakTime["100"];
    // Call category helper to get list of all categories
    categoryHelper(categoryList => {
      reprompt += categoryList;
  ...
  unhandledStartMode: function() {
    ...
    let message =
      "Sorry, to hear any news feed, you can select any category using its name or number. " +
      constants.breakTime["100"] +
      "Here are the available categories : " +
      constants.breakTime["100"];
    // Call category helper to get list of all categories
  ...
  unhandledFeedMode: function() {
    // Help user with possible options in _FEED_MODE
    const message =
      "Sorry, you can say next or previous to navigate through the feed. " +
      constants.breakTime["100"] +
      " To hear all categories, you can say, get category list." +
      constants.breakTime["100"] +
      " And say restart to start over the current feed. " +
  ...
  unhandledNoNewItemMode: function() {
    ...
    const message =
      "Sorry, you can say yes to hear older items and no to select other feeds.";
    const reprompt =
      this.attributes["category"] +
      " has no new items. " +
      "You can say yes to hear older items and no to select other feeds." +
  ...
  reportError: function() {
    ...
    const message =
      "Sorry, there are some technical difficulties in fetching the requested information. Please try again later.";
